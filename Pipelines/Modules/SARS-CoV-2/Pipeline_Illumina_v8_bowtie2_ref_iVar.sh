...
mkdir -p fastqc_raw
fastqc *_R1.fq.gz *_R2.fq.gz -o fastqc_raw -t "${THREADS}"

# 2. Limpeza das Reads
# Limpar reads usando Trimmomatic ou fastp
trimmomatic PE -phred33 ${F}_R1.fq.gz ${F}_R2.fq.gz  ${F}_R1_paired.fq.gz  ${F}_R1_unpaired.fq.gz  ${F}_R2_paired.fq.gz  ${F}_R2_unpaired.fq.gz  ILLUMINACLIP:$PIPELINE/SARS-CoV-2/primer_and_adapter_colection_v5.fasta:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:${TRIM_QUAL} MINLEN:36  -threads "${THREADS}" TOPHRED33 > ${F}_trimmomatic.log

# 3) FastQC dos dados pós-trim (use apenas os *_paired*)
mkdir -p fastqc_trimmed
fastqc ${F}_R1_paired.fq.gz ${F}_R2_paired.fq.gz -o fastqc_trimmed -t "${THREADS}"
...
bowtie2 -p "${THREADS}" -x $PIPELINE/SARS-CoV-2/Reference_Genomes/BA.2_ref -1 ${F}_R1_paired.fq.gz -2 ${F}_R2_paired.fq.gz -U ${F}_unpaired.fq.gz | samtools view -S -b -q 30 > ${F}_Wuhan_mapped.bam
...
pilon --genome $PIPELINE/SARS-CoV-2/Reference_Genomes/BA.2_ref.fasta --frags ${F}_Wuhan_mapped.sorted.bam --minmq 30 --output ${F}.Pilon --fix "gaps,indels" --mindepth $CONSENSUS_MIN_DEPTH --minqual $TRIM_QUAL;
...
bowtie2 -p "${THREADS}" -x ${F}_pilon_index -1 ${F}_R1_paired.fq.gz -2 ${F}_R2_paired.fq.gz -U ${F}_unpaired.fq.gz | samtools view -S -b -q 30 > ${F}.Pilon.bam
...
samtools mpileup -aa -A -d 0 -Q 0 ${F}.Pilon.sorted.bam | ivar consensus -q 30 -m ${CONSENSUS_MIN_DEPTH} -p ${F}_ivar -i ${F}.Pilon.fasta

# Ajuste de remapeamento
bowtie2-build ${F}_ivar.fa ${F}_ivar.fa


bowtie2 -p "${THREADS}" -x ${F}_ivar.fa  -1 ${F}_R1_paired.fq.gz -2 ${F}_R2_paired.fq.gz -U ${F}_unpaired.fq.gz | samtools view -S -b -q 30 > ${F}.Pilon2.bam
...
samtools mpileup -aa -A -d 0 -B -Q 0 ${F}.Pilon2.sorted.bam | ivar variants -p ${F}_iVar_variants -q 20 -t 0.25 -m $CONSENSUS_MIN_DEPTH -r ${F}_ivar.fa
...
if test -f ${F}_ref.algn.minor.fa; then
    ...
    pangolin -t "${THREADS}" Genoma_${F}_minor.fasta --outfile Pangolin__${F}_minor.tsv
