...
nextclade dataset get --name='sars-cov-2' --output-dir='/storage/zuleika/volume1/project/carol/sarsCov2/nextstrain_files/'

# Run NextClade 

mkdir -p ${H}__nextClade # Create NextClade output folder
nextclade run  --input-dataset /storage/zuleika/volume1/project/carol/sarsCov2/nextstrain_files/ --output-all ${H}__nextClade --output-basename nextclade --jobs ${P} ${F}

# Join NextClade results with Sorted All Stats using python
/usr/bin/python /storage/zuleika/volume1/project/carol/sarsCov2/nextstrain_files/process_AllStats_NextClade.py ${S:0:-4}_sorted.tsv ${H}__nextClade/nextclade.tsv
