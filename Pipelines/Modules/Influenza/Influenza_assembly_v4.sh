...
# ========================================================================
trimmomatic PE -phred33 ${F}_R1_cutadapt.fq.gz ${F}_R2_cutadapt.fq.gz ${F}_R1_paired.fq.gz ${F}_R1_unpaired.fq.gz ${F}_R2_paired.fq.gz ${F}_R2_unpaired.fq.gz LEADING:3 TRAILING:3 SLIDINGWINDOW:5:${TRIM_QUAL} MINLEN:35 TOPHRED33 -threads ${THREADS}
...
cat $PIPELINE/Influenza/segment_list.txt | xargs -P ${THREADS} -I {} sh -c 'N=$(cat sample_name.txt); $PIPELINE/Influenza/vapor/vapor.py -fa $PIPELINE/Influenza/segments_database/{}.fasta -fq "$N"_R1_paired.fq.gz "$N"_R2_paired.fq.gz | cut -f 6 | sed "s/>//g" > {}_result.txt'
...
if [ -s segment_7_result.txt ]; then
    grep -oP 'Influenza \K[AB]' segment_7_result.txt > A_or_B_genotype.txt
else
...
for i in 1 2 3 4 5 6 7 8; do
    ...
    if [ -s segment_${i}_result.txt ]; then
        ...
        bowtie2 --very-sensitive -p ${THREADS} -x segment_${i}_ref.fasta -1 ${F}_R1_paired.fq.gz -2 ${F}_R2_paired.fq.gz | samtools view -S -b -q 30 > segment_${i}_mapped.bam
        ...
        if test -f SPAdes_segment${i}/scaffolds.fasta; then
            ...
            samtools mpileup -aa -A -d 1000 -C 50 -Q 0 minimap_segment_${i}.sorted.bam | ivar consensus -p minimap_segment_${i}_preconsensus -m ${CONSENSUS_MIN_DEPTH} -q 0 -t 0
            ...
            bowtie2 --very-sensitive -p ${THREADS} -x segments/segment_preconsensus_${i}_${F}.fasta -1 segment_${i}_R1.fq.gz -2 segment_${i}_R2.fq.gz | samtools view -S -b -q 30 > segments/segment_preconsensus_${i}_${F}.bam
            ...
            bowtie2 --very-sensitive -p ${THREADS} -x segments/segment_${i}_${F}.fasta -1 segment_${i}_R1.fq.gz -2 segment_${i}_R2.fq.gz | samtools view -S -b -q 20 > segments/segment_${i}_${F}.bam
...
if [ -s Genoma_${F}.fasta ]; then
    ...
    bowtie2 --very-sensitive -p ${THREADS} -x Genoma_${F}.fasta -1 ${F}_R1_paired.fq.gz -2 ${F}_R2_paired.fq.gz | samtools view -S -b -q 30 > ${F}.bam
