...
# ----------------------------------------------------------------
# 6) Definir Passed_QC
#    - Se o arquivo consolidado já trouxer a coluna, usamos esse valor.
#    - Caso contrário, fazemos o fallback com base em segment_4_Coverage:
#      "A" se coverage >= 85
#      "E" se 50 <= coverage < 85
...
for index, row in statistics_df_useful.iterrows():
    ...
    try:
        if seg4_value in ['not_detected', 'could_not_be_assembled']:
            passed_qc_fallback[index] = 'R'
        else:
            val = float(seg4_value)
            if val >= 85:
                passed_qc_fallback[index] = 'A'
            elif 50 <= val < 85:
                passed_qc_fallback[index] = 'E'
            else:
                passed_qc_fallback[index] = 'R'
    except ValueError:
...
if 'Passed_QC' in statistics_df.columns:
    statistics_df_useful['Passed_QC'] = statistics_df['Passed_QC'].fillna(pd.Series(passed_qc_fallback))
else:
    statistics_df_useful['Passed_QC'] = pd.Series(passed_qc_fallback)
