...
def main():
    ...
    # Função para classificar QC quando o campo ainda não vier no arquivo consolidado
    ...
    if 'Passed_QC' in stats_full.columns:
        statistics_df_useful['Passed_QC'] = stats_full['Passed_QC']
    else:
    ...
    # Escolher explicitamente as colunas desejadas, priorizando os valores do arquivo consolidado
    def coalesce_columns(df, column_name):
        ordered_candidates = [
            f'{column_name}_right',
            f'{column_name}_left',
            column_name,
        ]
        selected = None
        for candidate in ordered_candidates:
            if candidate in df.columns:
                current = df[candidate]
                selected = current if selected is None else selected.combine_first(current)
        if selected is not None:
            df[column_name] = selected

    for column_name in ['Coverage', 'E_Coverage', 'Serotype', 'SNPs', 'Passed_QC']:
        coalesce_columns(merged, column_name)
