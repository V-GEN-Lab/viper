...
########################################
 pilon --genome $PIPELINE/DENV/reference_genomes/dengue_${selected_serotype}.fasta --frags ${F}_dengue_${selected_serotype}_mapped.sorted.bam --output ${F}_${selected_serotype}.Pilon --fix "gaps,indels" --threads ${THREADS} --mindepth $CONSENSUS_MIN_DEPTH --minqual $TRIM_QUAL --minmq 10
...
# -Q 0 Inclui todas as bases, independentemente da qualidade de base. O filtro de qualidade será aplicado depois, pelo iVar.
 ...
 samtools mpileup -aa -A -d 0 -Q 0 ${F}_dengue_${selected_serotype}_Pilon_mapped.sorted.bam | ivar consensus -m ${CONSENSUS_MIN_DEPTH} -p ${F}_ivar -i ${F}_pilon

########################################
# Etapa 8: Ajuste final de mapeamento
########################################
bwa index ${F}_ivar.fa # Indexa genoma consenso
bwa mem ${F}_ivar.fa ${F}_R1_paired.fq.gz ${F}_R2_paired.fq.gz -t ${THREADS} | samtools view -S -b -q 10 > ${F}.Pilon2.bam # Filtra por qualidade
...
########################################
samtools mpileup -aa -A -d 0 -B -Q 0 ${F}.Pilon2.sorted.bam | ivar variants -p ${F}_iVar_variants -q 20 -t 0.25 -m ${CONSENSUS_MIN_DEPTH} -r ${F}_ivar.fa # Parâmetros de filtro
...
GENES=("E" "NS1" "NS3" "NS5")
BED_DIR="$PIPELINE/DENV/gene_check"
...
########################################
nextclade run -D $PIPELINE/DENV/nextclade_files/denv${selected_serotype}/ -j ${THREADS} -t nextclade.tsv Genoma_${F}.fasta # Arquivo de sa?da
