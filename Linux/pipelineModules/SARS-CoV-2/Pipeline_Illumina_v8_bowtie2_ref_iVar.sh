...
trimmomatic PE -phred33 ${F}_R1.fq.gz ${F}_R2.fq.gz  ${F}_R1_paired.fq.gz  ${F}_R1_unpaired.fq.gz  ${F}_R2_paired.fq.gz  ${F}_R2_unpaired.fq.gz  ILLUMINACLIP:/storage/zuleika/volume1/project/carol/sarsCov2/CeVIVAS/pipeline/primer_and_adapter_colection_v5.fasta:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:20 MINLEN:36  -threads 10 > ${F}_trimmomatic.log
...
bowtie2 -p 5 -x /storage/zuleika/volume1/project/carol/sarsCov2/CeVIVAS/pipeline/Reference_Genomes/BA.2_ref -1 ${F}_R1_paired.fq.gz -2 ${F}_R2_paired.fq.gz -U ${F}_unpaired.fq.gz | samtools view -S -b -q 30 > ${F}_Wuhan_mapped.bam
...
java -jar /usr/local/src/pilon/1.24/pilon-1.24.jar --genome /storage/zuleika/volume1/project/carol/sarsCov2/CeVIVAS/pipeline/Reference_Genomes/BA.2_ref.fasta --frags ${F}_Wuhan_mapped.sorted.bam --minmq 30 --output ${F}.Pilon --fix "gaps,indels"  --threads 10 --mindepth 10;


# Correção do genoma com Pilon usando a versão do Picard
#java -jar /usr/local/src/pilon/1.24/pilon-1.24.jar --genome /storage/zuleika/volume1/project/carol/sarsCov2/CeVIVAS/pipeline/Reference_Genomes/BA.2_ref.fasta --frags marked_duplicates.bam --minmq 30 --output ${F}.Pilon --fix "gaps,indels" --threads 10 --mindepth 10;
...
python /storage/zuleika/volume1/project/carol/sarsCov2/CeVIVAS/pipeline/substitute_degenarate_bases.py ${F}_ivar.fa Genoma_${F}.fasta
...
python /storage/zuleika/volume1/project/carol/sarsCov2/CeVIVAS/pipeline/plot_depth_per_sample.py ${F}.GenomeDepth ${F}_Depth_plot.png ${F}
...
if [[ ! -s ${F}.Pilon2.sorted.bam ]]; then
    ...
    makeblastdb -in /storage/zuleika/volume1/project/carol/sarsCov2/CeVIVAS/pipeline/Reference_Genomes/BA.2_ref.fasta -dbtype nucl -out /storage/zuleika/volume1/project/carol/sarsCov2/CeVIVAS/pipeline/Reference_Genomes/BA.2_ref
    blastn -query Genoma_${F}.fasta -db /storage/zuleika/volume1/project/carol/sarsCov2/CeVIVAS/pipeline/Reference_Genomes/BA.2_ref -outfmt 6 > ${F}.blastn
...
mafft --keeplength --add Genoma_${F}_minor_check.fasta /storage/zuleika/volume1/project/carol/sarsCov2/CeVIVAS/pipeline/Reference_Genomes/BA.2_ref.fasta > ${F}_ref.algn
bam-readcount -w 0 -f /storage/zuleika/volume1/project/carol/sarsCov2/CeVIVAS/pipeline/Reference_Genomes/BA.2_ref.fasta -q 30 ${F}_Wuhan_mapped.sorted.bam  > ${mHeader}.fa.bc
python /storage/zuleika/volume1/project/carol/sarsCov2/CeVIVAS/pipeline/intrahost_script.py -in ${mHeader}.fa.bc -dp 100 -al ${F}_ref.algn
