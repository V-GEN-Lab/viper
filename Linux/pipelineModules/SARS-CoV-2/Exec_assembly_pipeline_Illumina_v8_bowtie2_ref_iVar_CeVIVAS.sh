...
cat Lista.txt | xargs -P ${THREADS} -I {} sh -c "bash $PIPELINE/SARS-CoV-2/Pipeline_Illumina_v8_bowtie2_ref_iVar.sh {} ${FOLDER}"; conda deactivate
...
#mv -f tree.json $PIPELINE/SARS-CoV-2/nextstrain_files/tree.json # move file to suport folder
...
conda activate pangolin
python $PIPELINE/SARS-CoV-2/write_cov_CeVIVAS_output8.py All_Statistics__${FOLDER}.csv $assemlby_path read_path.tsv $FOLDER
conda deactivate
