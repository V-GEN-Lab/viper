...
cutadapt -b file:$PIPELINE/DENV/primer_and_adapter_colection_dengue.fasta -B file:$PIPELINE/DENV/primer_and_adapter_colection_dengue.fasta -j 1 -o ${F}_R1_cutadapt.fq.gz -p ${F}_R2_cutadapt.fq.gz ${F}_R1.fq.gz ${F}_R2.fq.gz
...
do
    ...
    bwa mem -t 1 $PIPELINE/DENV/reference_genomes/dengue_${i}.fasta ${F}_R1_paired.fq.gz ${F}_R2_paired.fq.gz > ${F}_dengue_${i}_mapped.sam
